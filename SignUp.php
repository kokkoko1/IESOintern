<?php
//require  password_hash();   // password_hash()はphp 5.5.0以降の関数のため、バージョンが古くて使えない場合に使用
// セッション開始
session_start();
...
if (isset($_POST["signUp"])) {
    ...
    if (!empty($_POST["username"]) && !empty($_POST["password"]) && !empty($_POST["password2"]) && $_POST["password"] === $_POST["password2"]) {
        ...
        try {
            ...
            $stmt->execute(array($username, $password));  // パスワードのハッシュ化を行　わない　!（今回は文字列のみなのでbindValue(変数の内容が変わらない)を使用せず、直接excuteに渡しても問題ない）
